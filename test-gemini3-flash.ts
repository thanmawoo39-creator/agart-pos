async function testGemini3Flash() {
  console.log('🧪 Testing Gemini 3 Flash model...');
  ...
  try {
    ...
    // Test AI ask-business with Gemini 3 Flash
    ...
    if (aiResponse.ok) {
      ...
      try {
        ...
        if (aiJson.response !== 'AI is currently busy. Please try again in a few moments.') {
          console.log('🎉 Gemini 3 Flash is working!');
        } else {
          console.log('⚠️ Still falling back to default message - checking logs...');
        }
      } catch (e) {
    ...
    console.error('❌ Error testing Gemini 3 Flash:', error);
  }
}
